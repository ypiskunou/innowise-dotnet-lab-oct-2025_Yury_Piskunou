...
public class PostgreSqlTaskRepositoryTests : BaseIntegrationTest
{
    [Fact]
    public async Task AddAsync_ShouldInsertSingleTask()
    {
        var repository = new PostgreSqlTaskRepository(DbFactory);
        var newTask = new TaskItem(0, 
            "Тест добавления для Postgres", "Описание", false, DateTime.MinValue);
        ...
        addedTask.Title.Should().Be("Тест добавления для Postgres");
